<template>
    <m-flex column gap="6">
        <m-list v-if="loadingFiles || files?.meta.total" v-model:page="filesPage" :title="$t('files')" :items="files" :loading="loadingFiles">
            <template #item="{ item }">
                <mod-download :file="item" :mod="mod" type="file"/>
            </template>
        </m-list>

        <m-list v-if="loadingLinks || links?.meta.total" v-model:page="linksPage" :title="$t('links')" :items="links" :loading="loadingLinks">
            <template #item="{ item }">

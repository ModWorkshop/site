<template>
    <PopoverRoot v-if="type == 'dropdown'" v-model:open="open">
        ...
        <PopoverPortal>
            ...
            >
                <slot name="content"/>
                <PopoverArrow class="mws-dropdown-arrow"/>
            </PopoverContent>
        </PopoverPortal>
    </PopoverRoot>
        ...
        <TooltipRoot v-model:open="open">
            ...
            <Tooltip v-if="!disabled">
                <TooltipContent :side="side" :align="align" :class="computedClass" :side-offset="2" :collision-padding="32" :avoid-collisions="true">
                    <slot name="content"/>
                    <TooltipArrow class="mws-dropdown-arrow"/>
                </TooltipContent>
            </Tooltip>
        </TooltipRoot>
    </TooltipProvider>
...
const computedClass = computed(() => {
    if (Array.isArray(dropdownClass)) {
        dropdownClass.push('mws-dropdown');
    } else if (typeof dropdownClass === 'object') {
        dropdownClass['mws-dropdown'] = true;
    } else if (typeof dropdownClass === 'string') {
        return ['mws-dropdown', dropdownClass];
    } else {
        return 'mws-dropdown';
    }
});
...
.mws-dropdown[data-side='top'] {
    animation-name: slideDownAndFade;
}
.mws-dropdown[data-side='right'] {
    animation-name: slideLeftAndFade;
}
.mws-dropdown[data-side='bottom']  {
    animation-name: slideUpAndFade;
}
.mws-dropdown[data-side='left']{

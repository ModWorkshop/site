<template>
    <flex column gap="2" style="flex: 1;">
        ...
        <flex :column="column" :gap="gap" class="py-1">
...
const { items, column = true } = defineProps<{
    title?: string,
    gap?: number,
...
const total = computed(() => items?.meta?.total ?? 0);
const perPage = computed(() => items?.meta?.per_page ?? 1);

...
const tags = {
    ...
    "code": {
        type: 'content',
        replace: (attr, content) =>  `<pre><code class="language">${content.replace(/&nbsp;/g, ' ')}</code></pre>`,
        noParse: true,
        block: true
    },
    "php": {
        type: 'content',
        replace: (attr, content) =>  `<pre><code class="language-php">${content.replace(/&nbsp;/g, ' ')}</code></pre>`,
        noParse: true,
        block: true
    },
    'color': {
        ...
        replace: (attr, content) => {
            ...
            if(chroma.contrast(colorCode, checkColorBg) > 3.5)
...
export default class BBCodeParser {
    static _ignoreLoop(tagsMap, content){
        tagsMap.forEach(tag => {
    ...
    static _contentLoop(tagsMap, content){
        tagsMap.forEach(tag => {
            ...
            if(module.special || tag.closing){
                if(isIgnore)
                    content = this._ignoreTag(tag, module, content);
                else
                    content = this._processTag(tag, module, content);
            }
    ...
    static _tagLoop(tagsMap, cleanUp, parent){
        ...
        return tagsMap.map((tag) => {
            if(tag.children.length)
                tag.children = this._tagLoop(tag.children, cleanUp, tag.index);

            return tag;
        });
    };
    
    static _processTag(tag, module, content){            
    ...
    static _ignoreTag(tag, module, content){
        let openTag = "[TAG-" + tag.index + "]";
        let start = content.indexOf(openTag);
        let closeTag = "";
    ...
    static registerTag(tag, options){
        tags[tag.toLowerCase()] = options;
        return this;
    };
    ...
    static process(input){
        ...
        const cleanUp = [];
        tagsMap = this._tagLoop(tagsMap, cleanUp);

        // put back all non-found matches?
        input = this._contentLoop(tagsMap, input);
        input = this._ignoreLoop(cleanUp, input); //Revert tags that failed to parse.
